...
def matrix(*rows, device=None):
    ...
    elems = [
        (
            ...
            else constant(x, shape=ref[0].shape, device=ref[0].device)
        )
        for x in elems
    ]
...
class Augmentations:
    ...
    def __init__(self, conf, eps_aug=1e-6):
        self.prob = conf.prob
        aux = conf.types
        self.aug = AugmentPipe(
            p=self.prob,
            xflip=aux.xflip,
            yflip=aux.yflip,
            rotate_int=aux.rotate_int,
            translate_int=aux.translate_int,
            scale=aux.scale,
            rotate_frac=aux.rotate_frac,
            aniso=aux.aniso,
            translate_frac=aux.translate_frac,
            brightness=aux.brightness,
            contrast=aux.contrast,
            lumaflip=aux.lumaflip,
            hue=aux.hue,
            saturation=aux.saturation,
        )
        self.skip_xflip = aux.xflip >= 1000
        self.eps_aug = eps_aug

    def __call__(self, x):
        x, a = self.aug(x)
        if self.skip_xflip:
            a[:, 0] *= 0
        a = (a.abs().sum(1) > self.eps_aug).long()
        return x, a
